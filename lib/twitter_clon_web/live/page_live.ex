defmodule TwitterClonWeb.PageLive do
  ...
  def mount(_params, _session, socket) do
    {:ok, assign(socket, counters: gen_counters(1), word: "", results: [])}
  end
  ...
  def handle_event("add", %{"w" => word}, socket) do
    if(word != "",
      do: {:noreply, assign(socket, results: add_words(socket.assigns.results, word))},
      else:
        {:noreply,
         socket
         |> put_flash(:error, "The word can't be blank")
         |> assign(results: socket.assigns.results)}
    )
  end
